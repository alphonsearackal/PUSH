...
static void fill_ipv6_header(pkt_gen_configuration_t *configuration)
{
	char system_command[MAX_SYS_CMD_LEN] = { 0 };
	char dialog_output[MAX_OUTPUT_LEN] = { 0 };
	const int checks_MAX_ITEMS = 4;
	const int checks_DIALOG_HEIGHT = 12;
	char dialog_output_1[MAX_OUTPUT_LEN] = { 0 };
	const int _IP6_VERSION = 6;
	const int _IP6_HLIM_DEFAULT = 64;
	uint8_t class = 0;
	uint8_t label = 0;
	struct in6_addr ip6_src = { .s6_addr32 = { 0, 0, 0, 0} };
	struct in6_addr ip6_dst = { .s6_addr32 = { 0, 0, 0, 0} };

	DEBUG_PRINT(DEBUG_INFO, "Filling IPv4 Protocol header.");

	/* Fill default values */
	configuration->pkt_len = ETH_HLEN + (configuration->check_tagged ? 4 : 0) + 
		sizeof(struct ip6_hdr) + sizeof(struct udphdr) + MIN_PAYLOAD_LEN; 
	configuration->data_len = configuration->pkt_len - (sizeof(struct ip6_hdr)
			+ ETH_HLEN + (configuration->check_tagged ? 4 : 0));

	configuration->ipv6_header.ip6_flow = htonl ((_IP6_VERSION << 28) | (class << 20) | (label));
	configuration->ipv6_header.ip6_plen = htons(configuration->data_len);
	configuration->ipv6_header.ip6_nxt = IPPROTO_UDP;
	configuration->ipv6_header.ip6_hops = _IP6_HLIM_DEFAULT;
	/*	inet_pton (AF_INET6, ip6_src, &configuration->ipv6_header.ip6_src);
		inet_pton (AF_INET6, ip6_dst, &configuration->ipv6_header.ip6_dst);
	 */	
	configuration->udp_header.source = htons(4444);
	configuration->udp_header.dest = htons(5555);
	configuration->udp_header.len = htons(configuration->data_len);
	configuration->udp_header.check = 0;

	sprintf(system_command, "%s --backtitle \"P.U.S.H.\" --title \"IPv6 header\""
			" --checklist \"Choose fields you want to modify\" %d 40 %d",
			DIALOG_BIN, checks_DIALOG_HEIGHT, checks_MAX_ITEMS);
	strcat(system_command, " 0 Flow-label off");
	strcat(system_command, " 1 Source-IP off");
	strcat(system_command, " 2 Dest-IP off");
	strcat(system_command, " 3 Next-header off");
...
static void fill_ipv4_header(pkt_gen_configuration_t *configuration)
{
	...
	/* Fill default values */
	configuration->pkt_len = ETH_HLEN + (configuration->check_tagged ? 4 : 0) + 
		sizeof(struct iphdr) + sizeof(struct udphdr) + MIN_PAYLOAD_LEN; 
...
void fill_protocol(pkt_gen_configuration_t *configuration)
{
	...
	switch(atoi(dialog_output))
	{
		...
		case 2:
			{
				configuration->protocol = proto_ipv6;
				fill_ipv6_header(configuration);
			}
		default:
			break;
	}
...
void fill_input_checks(pkt_gen_configuration_t *configuration)
{
	...
	const int checks_DIALOG_HEIGHT = 14;
...
{
	char system_command[MAX_SYS_CMD_LEN] = { 0 };
	const int checks_DIALOG_HEIGHT = 5;
